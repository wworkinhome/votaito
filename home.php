				    ...
				    <?php
				    	...
				    	if($vquery->num_rows > 0){
				    		...
				    		<div class="text-center">
					    		<h3>Usted ya ha votado en estas elecciones.</h3>
					    		<a href="#view" data-toggle="modal" class="btn btn-flat btn-primary btn-lg">Ver votación</a>
					    	</div>
				    		<?php
				    	}
				    	else{
						    ...
						    <form method="POST" id="ballotForm" action="submit_ballot.php">
				        		<?php
									...
									while($row = $query->fetch_assoc()){
										...
										$instruct = ($row['max_vote'] > 1) ? 'Puede seleccionar hasta '.$row['max_vote'].' candidates' : 'Seleccione solo un candidato';
...
$(function(){
	...
	$('#preview').click(function(e){
		...
		if(form == ''){
			$('.message').html('Debe votar al menos por un candidato');
			$('#alert').show();
		}
		else{
			$.ajax({
